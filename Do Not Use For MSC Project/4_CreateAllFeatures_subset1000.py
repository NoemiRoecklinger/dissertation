...
subset_test_ifPCbreaksDown = dataxyz[:1000]
dataxyz = None 
...
idx_list = kdt.query(subset_test_ifPCbreaksDown, k=10, return_distance=False)
store = []
...
df2 = pd.DataFrame({
    ...
    'Y': store[:,1],
    'Z': store[:,2],
    'range': df['relative_height'].iloc[:1000],
    'verticality': df['verticality'].iloc[:1000],
    'lambda1': store[:,3],
    'lambda2': store[:,4],
    'lambda3': store[:,5],
    'lambda4': store[:,6],
